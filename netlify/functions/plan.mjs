...
Use headings on separate lines.
Begin directly with the heading "Trip Overview:". Do not repeat the
destination, duration, budget, traveller or interests at the beginning.
After the overview, include the heading "Recommended Stay Areas:".
Start every day section with "Day 1:", "Day 2:" and so on.
Add a short descriptive title after each day number, such as
"Day 1: Heritage Streets and Waterfront".
Use the labels "Morning:", "Afternoon:" and "Evening:" on separate lines.
Under each Morning, Afternoon and Evening label, use simple hyphen bullets.
Begin every activity bullet with a short, attractive two-to-five-word title,
followed by a colon and then the explanation.
Example: "Sunrise Harbour Walk: Begin beside the waterfront before the crowds."
Use the same short-title format for hotels, food, transport, packing items and
budget recommendations so the PDF can display those titles in bold.
Return plain text only. Do not use Markdown hash symbols, asterisks,
underscores, code fences or tables.
