...
pdfButton.addEventListener("click", () => {
    ...
    function drawParagraph(text, isBullet = false) {
        ...
        const lineWidth = contentWidth - leftIndent;

        pdf.setFont("helvetica", "normal");
        pdf.setFontSize(10);
        setTextColour(colours.text);

        const wrappedLines = pdf.splitTextToSize(cleanText, lineWidth);
        const blockHeight = wrappedLines.length * 5.2 + 2;
        ensureSpace(blockHeight);

        if (isBullet) {
            setFillColour(colours.coral);
            pdf.circle(margin + 1.5, yPosition - 1.2, 1, "F");
        }

        pdf.text(wrappedLines, margin + leftIndent, yPosition);
    ...
    function isHeading(line) {
        ...
        return (
            /^#{1,6}\s+/.test(line) ||
            /^\*\*[^*]+\*\*:?\s*$/.test(line) ||
            /^(day\s+\d+|trip overview|overview|where to stay|recommended hotels?|hotels?|local food|food suggestions?|transport advice|transportation|packing list|estimated budget|budget breakdown|useful local tips|local tips)$/i.test(
                cleaned
            )
        );
    }
    ...
    // Convert the AI's Markdown-like response into styled PDF sections.
    latestItinerary.split(/\r?\n/).forEach((rawLine) => {
        ...
        if (bulletMatch) {
            drawParagraph(bulletMatch[1], true);
        } else if (numberedMatch) {
            drawParagraph(line, true);
        } else {
            drawParagraph(line);
        }
    });
