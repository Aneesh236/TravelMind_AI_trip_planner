...
// Load one of the travel artworks before creating the PDF.
function loadPdfImage(fileName, testImage) {
    if (testImage) {
    ...
    try {
        ...
        try {
            const imageResults = await Promise.allSettled([
                loadPdfImage(
                    "pdf-travel-cover.jpg",
                    window.__PDF_TEST_COVER_IMAGE
                ),
                loadPdfImage(
                    "pdf-cover-background.jpg",
                    window.__PDF_TEST_BACKGROUND_IMAGE
                )
            ]);
            coverImageData =
                imageResults[0].status === "fulfilled"
                    ...
                    : null;
            backgroundImageData =
                imageResults[1].status === "fulfilled"
        ...
        const colours = {
            ...
            blue: [56, 105, 135],
            paleBlue: [231, 241, 247],
            purple: [112, 79, 139],
            palePurple: [242, 235, 248],
            text: [42, 54, 51],
            muted: [104, 115, 111],
            white: [255, 255, 255],
            line: [221, 226, 222]
        ...
        function drawHeading(text) {
            ...
            if (/hotel|stay|accommodation/.test(headingLower)) {
                activeTheme = {
                    accent: colours.green,
                    pale: colours.paleGreen
                };
            } else if (/food|meal|restaurant/.test(headingLower)) {
                activeTheme = {
                    accent: colours.coral,
                    pale: colours.paleCoral
                };
            } else if (/transport|getting around/.test(headingLower)) {
                activeTheme = {
                    accent: colours.blue,
                    pale: colours.paleBlue
                };
            } else if (/packing/.test(headingLower)) {
                activeTheme = {
                    accent: colours.purple,
                    pale: colours.palePurple
                };
            } else if (/budget|cost/.test(headingLower)) {
                activeTheme = {
                    accent: colours.gold,
                    pale: colours.paleGold
                };
            } else {
                activeTheme = {
                    accent: colours.gold,
            ...
            ensureSpace(18);
            setFillColour(activeTheme.pale);
            pdf.roundedRect(margin, yPosition, contentWidth, 13, 2.5, 2.5, "F");
            setFillColour(activeTheme.accent);
        ...
        function drawDayHeading(text) {
            ...
            const dayTitle = dayMatch?.[2] || "Daily itinerary";
            pdf.setFont("helvetica", "bold");
            pdf.setFontSize(14);
        ...
        function drawBullet(text, number = null) {
            ...
            setFillColour(number === null ? activeTheme.pale : colours.paleCoral);
            setDrawColour(colours.line);
            pdf.roundedRect(
                margin,
                yPosition,
            ...
            setFillColour(number === null ? activeTheme.accent : colours.gold);
            ...
            if (labelMatch) {
                ...
                setTextColour(activeTheme.accent);
        ...
        setFillColour(colours.coral);
        pdf.roundedRect(margin, 18, 49, 10, 5, 5, "F");
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(8.5);
        setTextColour(colours.white);
        pdf.text("TRAVEL MIND", margin + 24.5, 24.5, { align: "center" });

        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(31);
        setTextColour(colours.white);
        const destinationTitle = pdf
            .splitTextToSize(latestTrip.destination, 100)
            .slice(0, 3);
        pdf.text(destinationTitle, margin, 49);
        ...
        pdf.text(
            ...
            margin,
            56 + (destinationTitle.length - 1) * 12
        );

        setDrawColour(colours.coral);
        pdf.setLineWidth(1.2);
        pdf.line(margin, 90, margin + 34, 90);
        pdf.setFont("helvetica", "normal");
        pdf.setFontSize(10);
        setTextColour(colours.white);
        pdf.text("A CURATED GUIDE CREATED FOR", margin, 99);
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(14);
        setTextColour(colours.gold);
        pdf.text(latestTrip.traveller.toUpperCase(), margin, 108);

        setFillColour(colours.darkGreen);
        pdf.roundedRect(margin, 238, 94, 35, 4, 4, "F");
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(8);
        setTextColour(colours.coral);
        pdf.text("YOUR TRAVEL STYLE", margin + 7, 247);
        pdf.setFontSize(12);
        setTextColour(colours.white);
        pdf.text(
            `${latestTrip.budget}  |  ${latestTrip.interests.join(" - ")}`,
            margin + 7,
            256,
            { maxWidth: 80 }
        );
        pdf.setFont("helvetica", "normal");
        pdf.setFontSize(8.5);
        setTextColour(colours.paleGold);
        pdf.text(
            "Designed by Travel Mind",
            margin + 7,
            266
        );

        // Page 2: summary and introductory sections.
        addNewPage();
        drawHeading("Your trip at a glance");

        const summaryTop = yPosition;
        setFillColour(colours.paleGreen);
        pdf.roundedRect(margin, summaryTop, contentWidth, 49, 4, 4, "F");
        ...
        latestItinerary.split(/\r?\n/).forEach((rawLine) => {
            ...
            if (
                /\b\d+\s*[- ]day\s+itinerary\b/i.test(content) ||
                /^budget\s*:.*(travell|interest)/i.test(content)
            ) {
                return;
            }
            ...
            if (
                ...
                if (
                    hasStartedDays &&
                    /^(recommended|where to stay|local food|transport|packing|estimated budget|useful local tips)/i.test(
                        content
                    ) &&
                    yPosition > 170
                ) {
